 ...
 *
 * SR110 AI curriculum Lab 02 - Camera Capture Basics.
 * Adapted as-is from zephyr_srsdk/samples/drivers/video/mipi_capture
 * (only CMakeLists.txt/Kconfig were trimmed down to a WQVGA-only lab;
 * this source file is unmodified from the original SDK sample).
 */
...
static int store_captured_frame(const struct video_buffer *vbuf, bool retain_capture_buffer);
...
static int store_captured_frame(const struct video_buffer *vbuf, bool retain_capture_buffer)
{
        ...
        return 0;
#else
        if (retain_capture_buffer) {
                stored_frame = vbuf->buffer;
                stored_frame_size = vbuf->bytesused;
                LOG_INF("Stored captured frame in-place: %u bytes at %p",
                        (uint32_t)stored_frame_size, stored_frame);
                return 0;
        }
...
int main(void)
{
        ...
        struct video_buffer *retained_buffer = NULL;
#if IS_ENABLED(CONFIG_VIDEO_SAMPLE_RES_FHD)
        ...
        TS_LOG("MIPI_CAPTURE_MAIN_ENTER");
        ...
        ret = video_import_buffer(shm_vbuf.buffer, shm_vbuf.size, &imported_idx);
        if (ret != 0) {
                LOG_ERR("video_import_buffer(SHM) failed: %d", ret);
                final_ret = ret;
                goto out_cleanup_buffers;
        }
        shm_vbuf.index = imported_idx;
        ...
        video_started = true;
        LOG_INF("Video stream started in %lld ms",
        ...
        for (uint32_t frame_idx = 0U; frame_idx < VIDEO_SAMPLE_CAPTURE_COUNT; frame_idx++) {
                ...
                LOG_INF("Frame %u captured: bytesused=%u timestamp=%u buffer=%p",
                        ...
                        captured->buffer);
                LOG_INF("Frame %u captured and available in RAM in %lld ms",
                        (uint32_t)(frame_idx + 1U), (long long)app_elapsed_ms(capture_start_ms));
                if (frame_idx == 0U) {
                        TS_LOG("MIPI_CAPTURE_FIRST_RAW_FRAME_IN_RAM");
                }

                bool retain_capture_buffer = !IS_ENABLED(CONFIG_VIDEO_SAMPLE_RES_FHD) &&
                                       ((frame_idx + 1U) == VIDEO_SAMPLE_CAPTURE_COUNT);

                ret = store_captured_frame(captured, retain_capture_buffer);
                if (ret < 0) {
                ...
                LOG_INF("Frame %u stored for sample access in %lld ms",
                        (uint32_t)(frame_idx + 1U), (long long)app_elapsed_ms(capture_start_ms));

                if (retain_capture_buffer) {
                        retained_buffer = captured;
                        captured = NULL;
                        continue;
                }
...
out_cleanup_buffers:
        ...
        if (!IS_ENABLED(CONFIG_VIDEO_SAMPLE_RES_FHD)) {
                for (size_t i = 0; i < ARRAY_SIZE(queued_buffers); i++) {
                        if ((queued_buffers[i] != NULL) && (queued_buffers[i] != retained_buffer)) {
