 ...
 *
 * Lab C4 (Camera JPEG Encode + TFT Still Preview): this is the SDK's
 * samples/drivers/video/enc sample (live sensor mode) with one
 * addition - after the hardware encoder produces a JPEG frame, it is
 * decoded back to RGB565 on-device and shown as a still image on the
 * ST7789V3 TFT (see tft_still.c/.h). Everything else below is
 * unmodified from the original sample.
 */
...
static int seed_memory_frame(void)
{
	...
	/*
	 * In mode=1 (memory-input), the driver places the JPEG output region after
	 * the raw input region. The application only needs to populate the raw
	 * input frame at frame-raw-offset.
	 */
...
static int wait_and_report(const struct device *enc, uint32_t mode, int64_t encode_start_ms)
{
	struct video_buffer *out = NULL;
	int ret;

	ARG_UNUSED(mode);
	TS_LOG("ENC_JPEG_WAIT_START");
	...
	TS_LOG("ENC_JPEG_DONE");
	LOG_INF("Encoder done in %lld ms (jpeg=%u bytes, mode=%s)",
		(long long)app_elapsed_ms(encode_start_ms),
		(unsigned int)out->bytesused, mode_name(mode));
	...
	if (out->bytesused != 0U) {
		...
		/*
		 * New in this lab (vs. the original SDK sample): decode the
		 * JPEG we just produced and show it as a still image on the
		 * TFT, while the buffer is still valid (it gets recycled by
		 * cleanup_capture()/drain_buffers() right after we return).
		 */
		int disp_ret = tft_still_show_jpeg(out->buffer, out->bytesused);

		if (disp_ret != 0) {
			LOG_WRN("tft_still_show_jpeg failed ret=%d (display wiring/decode issue?)",
				disp_ret);
		}
	}
...
static int run_live_mode(const struct device *enc)
{
	...
	enc_started = true;
	LOG_INF("Encoder stream started in %lld ms",
		(long long)app_elapsed_ms(encoder_phase_start_ms));

	ret = wait_and_report(enc, APP_MODE_SENSOR_TO_MEMORY, k_uptime_get());
	if (ret != 0) {
		LOG_ERR("live-mode JPEG capture failed ret=%d", ret);
	} else {
		LOG_INF("Live-mode encode flow completed in %lld ms",
			(long long)app_elapsed_ms(encoder_phase_start_ms));
	}
...
static int run_memory_mode(const struct device *enc)
{
	...
	/*
	 * Note: in this mode the source is a synthetic 960x540 pattern,
	 * not this lab's fixed 480x270 live size, so tft_still_show_jpeg()
	 * (sized for 480x270 -> 240x135 decode) will log a size mismatch
	 * warning and skip the display step - only mode=0 (the default)
	 * drives the TFT still preview.
	 */
	ret = wait_and_report(enc, APP_MODE_MEMORY_INPUT, k_uptime_get());
	if (ret != 0) {
		LOG_ERR("reserved-memory JPEG capture failed ret=%d", ret);
	} else {
		LOG_INF("Memory-input encode flow completed in %lld ms",
			(long long)app_elapsed_ms(encoder_phase_start_ms));
	}
...
int main(void)
{
	...
	LOG_INF("Sample completed in %lld ms from start",
